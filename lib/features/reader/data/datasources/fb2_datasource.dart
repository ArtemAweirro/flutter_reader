...
class _Fb2ParseParams {
  ...
  const _Fb2ParseParams({required this.bookId, required this.filePath});
}
...
class Fb2DataSource {
  Future<ReaderBookEntity> openFb2(int bookId, String filePath) async {
    ...
    return compute(
      _parseFb2InIsolate,
      _Fb2ParseParams(bookId: bookId, filePath: filePath),
    );
  }
}

Future<ReaderBookEntity> _parseFb2InIsolate(_Fb2ParseParams params) async {
...
String _parseTitle(XmlDocument doc) {
  return doc
      ...
      .firstWhere((t) => t.isNotEmpty, orElse: () => 'Без названия');
}
...
void _collectSections(XmlElement section, List<ChapterEntity> result) {
